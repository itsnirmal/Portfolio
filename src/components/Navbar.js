  ...
  return (
    <div className="bg-zinc-900 w-screen h-14 top-0 z-3 absolute flex flex-wrap md:flex-nowrap items-center">
      <Link className="block px-14 py-auto md:py-0 order-1" to="/">
        <span className="transition-all ease-in-out delay-150 block mx-8 my-auto font-bold text-2xl hover:scale-110">itsnirmal</span>
      </Link>
      <button onClick={toggleMenu} className="transition-all ease-in-out delay-150 hover:scale-125 md:hidden text-white p-2 absolute right-8">
        {showMenu ? <FiX /> : <FiMenu />}
      </button>
